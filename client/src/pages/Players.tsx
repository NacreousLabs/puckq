  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-display font-bold text-white mb-2">Players Database</h1>
                  ...
                  <tr key={player.id} className="hover:bg-white/5 transition-colors group cursor-pointer">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span className="opacity-70">{team?.logo}</span>
                        {team?.abbreviation}
                      </div>
                    </td>
                    <td className="px-6 py-4">{player.position}</td>
                    <td className="px-6 py-4 text-muted-foreground">{player.age}</td>
                    <td className="px-6 py-4 font-mono text-emerald-400/90">{formatCurrency(player.capHit)}</td>
