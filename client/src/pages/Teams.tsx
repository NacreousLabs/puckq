      ...
      {filteredTeams.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredTeams.map((team: Team) => {
            ...
            return (
              <Link key={team.id} href={`/teams/${team.id}`} className="glass-panel rounded-xl p-5 border-border/40 hover:border-primary/30 transition-all group cursor-pointer bg-card flex flex-col justify-between no-underline text-inherit" data-testid={`card-team-${team.id}`}>
                <div>
